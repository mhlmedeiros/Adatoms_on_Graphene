...
## CLASSES TO GENERATE THE SHAPES OF THE SYSTEMS:
class Rectangle:
...
def hopping(site1, site2, t, B, peierls):
    """
    This function defines the hopping integral
    between two closest neighbors. It is suited
    for 2D systems subjected to a perpendicular
    magnetic field.
    """
...
def peierls(site1, site2, B):
    """
    Definition of Peierls phase for graphene sublattice
    with |a_i| = 1 (Bravais lattice vector magnitude):
        - e     == 1
        - \hbar == 1

    Here the gauge adopted is
        A = [[-yB],
             [ 0 ],
             [ 0 ]]
    which is suited for finite systems and leads
    with translation symmetry in x-direction.

    """
    xi, yi = site1.pos    # target
    xj, yj = site2.pos    # source
    area = np.sqrt(3)/2   # unit cell area
...
def peierls_vertical(site1, site2, B):
    ...
    Here the gauge adopted is
        A = [[ 0 ],
             ...
             [ 0 ]]
    which is suited for finite systems and leads
    with translation symmetry in y-direction.
    """
    xi, yi = site1.pos # target
    xj, yj = site2.pos # source
    area = np.sqrt(3)/2
...
if __name__ == "__main__":
    ...
    for Bflux in Bflux_values:
        parameters = dict(t=1, B=Bflux, V=0, peierls=peierls)
